...
- ~~Rob wants to refactor the duplication for summing with either a comma or newline, but we are deciding to leave it
  for now~~ because no longer relevant
- Trying to focus on parsing instead of validating, but feel uneasy about not validating (we are not supposed to be
  validating based on doc)
